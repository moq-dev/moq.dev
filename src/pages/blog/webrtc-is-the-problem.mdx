...
OpenAI [posted a technical blog](https://openai.com/index/delivering-low-latency-voice-ai-at-scale/) post a few days ago.
...
**But I'm not not allowed to wait**.
It's *impossible* to even retransmit an audio packet with WebRTC; we tried at Discord.
The protocol is hard-coded for real-time latency **or else**.
...
Like seriously, timestamps are ignored.
It's even more annoying when video is introduced because WebRTC has no A/V sync.

To compensate for this, OpenAI has to make sure packets arrive *exactly* when they should be rendered.
They have to **add a sleep** in front of every audio packet **before sending it**.
...
<figure>
	...
	<figcaption>Thank for Mr Robot friend for the unambigious advice</figcaption>
</figure>
...
But as OpenAI corroberates, this causes issues at scale because...
...
It's [not hard](https://datatracker.ietf.org/doc/html/rfc5764) to figure out which protocol a packet is using, but we need figure out how to route each packet.
...
<figure>
	...
	<figcaption>Personally, I would prefer 3 racoons.</figcaption>
</figure>
...
<figure>
	...
	<figcaption>It's an [obscure reference](https://www.youtube.com/watch?v=catXIV3zAGY) to an obscure reference. wow so meta</figcaption>
</figure>
...
But I think should also throw the baby out with the bath water.
...
It makes for a boring blog post, but it's simple and works at OpenAI scale.

The day might come when you want to drop audio packets.
Or you want to transmit video over the same connection.
Then you can switch to QUIC/WebTransport, because...
...
Instead, every packet instead contains a `CONNECTION_ID`, which can be 0-20 bytes long.
...
The backend server completes the handshake and **encodes it's own ID** into the `CONNECTION_ID`.
...
<figure>
	...
	<figcaption>Your argument is irrelevant, as I have labeled QUIC as the chad.</figcaption>
</figure>

## To Be Fair
I know many engineers at OpenAI and they are extremely bright.
They're dealing with an unprecedented levels of stress.
...
It's very easy for me judge from my lofty position, like a movie critic ranting about how *they casted Jared Leto again*?
