...
| ----------------------- | -------- | --------- |
| Raw                     | 48816 B  | —         |
| DEFLATE only            | 29791 B  | 39%       |
| JSON Merge Patch only   | 20430 B  | 58%       |
| **Both**                | **4181 B** | **91%** |

Deltas + compression stack into a ~12x reduction on the wire.

The shared compression window pulls its weight too.
Reusing one sync-flushed DEFLATE window across the snapshot+delta frames beats spinning up a fresh encoder per frame, and it's *faster* to boot:

| Strategy                  | Size     | Encode + decode |
| ------------------------- | -------- | --------------- |
| Fresh encoder per frame   | 13829 B  | 5853 µs/run     |
| **Shared sync-flush window** | **4249 B** | **4333 µs/run** |

~3.25x smaller wire size, no encoder rebuild per frame.
