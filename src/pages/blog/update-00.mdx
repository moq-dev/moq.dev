...
| **JSON Merge Patch + DEFLATE per group** | **89%**       |

Deltas + compression stack into a ~9x reduction on the wire.

And the compression *window* matters: sharing one DEFLATE window across a whole group beats compressing each frame on its own (89% vs 72% saved, ~2.7x smaller), since every frame gets to reuse the ones before it.
