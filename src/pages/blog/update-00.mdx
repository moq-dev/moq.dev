...
- Raw: `48816 B`
- DEFLATE only: `29791 B` (**39%** smaller)
- JSON Merge Patch only: `20430 B` (**58%** smaller)
- Both together: `4181 B` (**91%** smaller)

Deltas + compression stack into a ~12x reduction on the wire.

The shared compression window pulls its weight too.
Reusing one sync-flushed DEFLATE window across the snapshot+delta frames beats spinning up a fresh encoder per frame: `4249 B` vs `13829 B`, about **3.25x** smaller.
And it's *faster* (4333 vs 5853 µs/run) since you're not rebuilding the encoder every frame.
